...
{
    internal class SaveAllCommand : ICommand
    {
        CarDirectory_ViewModel viewModel;
        public event EventHandler? CanExecuteChanged;

        public SaveAllCommand(CarDirectory_ViewModel vm)
        {
            this.viewModel = vm;
        }
        ...
        Environment.SpecialFolder.Desktop), "cars-" + DateTime.Now.ToString().Replace(':', '-') + ".json");
                using (FileStream fs = new FileStream(fileName, FileMode.CreateNew))
                {
                    ...
                    fNode.CarInfo = viewModel.Cars.ToList();
            ...
            {
                MessageBox.Show("На данный момент нет открытх автомобилей");
